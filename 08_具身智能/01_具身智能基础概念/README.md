...
本章只建立后续学习所需的概念坐标系，重点讲清智能体、环境、本体、观测、状态、策略、动作、反馈与奖励之间的关系。运动学、控制、机器人学习、VLA 和世界模型的具体方法，将分别在后续章节展开。

本章适合已经了解基本 AI 概念、但尚未系统接触机器人与具身智能的读者，也可以作为阅读具身智能论文和系统资料前的术语索引。

## 读完本章，你应该能够

- 用自己的语言解释“具身”和“具身智能”。
- 区分智能体、环境、本体、观测、状态、策略、动作、反馈和奖励。
...
<div align="center">
<img src="./images/01-embodied-ai-knowledge-map.webp" alt="具身智能基础概念知识地图" />
<br>
...
| [交互闭环](#interaction-loop) | 智能体如何从感知走向行动？ | 观测、状态、策略、动作、反馈、奖励 |
...
<div align="center">
<img src="./images/02-body-shapes-intelligence.webp" alt="身体塑造具身智能的感知、动作与交互经验" />
<br>
...
具身系统的观测并不只有视觉。相机、深度传感器和激光雷达提供对外部环境的感知；关节位置、速度、力矩和机身姿态等**本体感受（proprioception）**，则帮助智能体了解自身当前处于什么状态。二者共同决定系统能够如何理解场景并控制身体。
...
- **具身认知（embodied cognition）** 更侧重一种认知观点：认知受到身体及其感觉运动经验的塑造。Margaret Wilson 的 [*Six Views of Embodied Cognition*](https://doi.org/10.3758/BF03196322) 对这一观点的不同含义进行了代表性梳理。
...
> **术语说明：** 本文将机器人身体及其传感—执行接口简称为“本体”。英文语境中的 *embodiment* 还可以表示身体、环境与智能之间更广义的具身关系，不应只理解为一套硬件外壳。
...
<div align="center">
<img src="./images/03-agent-environment-interaction-loop.webp" alt="环境、本体与智能体之间的观测和动作闭环" />
<br>
...
<div align="center">
<img src="./images/04-observation-vs-state.webp" alt="真实状态与当前观测的区别，以及主动换视角获得新观测" />
<br>
...
这套表达只用于建立概念关系。本章不要求推导 POMDP，后续机器人学习和世界模型章节会继续展开相关方法。关于部分可观测决策过程的经典定义，可参考 Kaelbling、Littman 与 Cassandra 的 [*Planning and Acting in Partially Observable Stochastic Domains*](https://doi.org/10.1016/S0004-3702%2898%2900023-X)。

## 策略如何把观测转化为动作？

**一句话理解：策略规定智能体在当前信息和目标条件下应该选择什么动作。**

本章使用广义的“策略（policy）”描述从当前信息到行动选择的决策机制。它可以由规则、规划器、控制器、神经网络或多个模块组成，不等同于“大模型”，也不一定通过强化学习获得。在强化学习语境中，策略通常更严格地指状态或观测到动作分布的映射。
...
**一句话理解：反馈是动作之后获得的广义信息，奖励则是部分学习方法用来表达目标偏好的数值信号。**

| 概念 | 它是什么 | 具身任务中的例子 |
| --- | --- | --- |
| 反馈（feedback） | 动作执行后，系统获得的观测、测量或结果信息 | 杯子位置变化、夹爪接触力、抓取成功与否、人类纠正 |
| 奖励（reward） | 强化学习中评价当前结果、引导策略优化的标量信号 | 成功放入托盘获得正奖励，发生碰撞得到负奖励 |

闭环执行需要利用反馈，但不一定依赖奖励；模仿学习、规划和人工控制系统同样可以形成反馈闭环。反过来，奖励也只是训练目标的一种表达方式：奖励设计不合理时，智能体可能获得高分，却没有完成设计者真正期望的任务。
...
<div align="center">
<img src="./images/05-open-vs-closed-loop.webp" alt="开环执行抓空与闭环反馈修正的对比" />
<br>
...
- 反馈是行动后获得的广义信息，奖励是部分学习方法用于优化策略的标量信号，二者不能等同。
...
9. 反馈和奖励有什么区别？为什么闭环系统不一定需要奖励？

<details>
<summary>展开参考答案要点</summary>

1. 拥有身体只说明系统具有物理载体；具身智能还要求它利用观测、行动和环境反馈完成任务或调整行为。
2. 状态描述世界实际的情况，观测只包含传感器当前获得的信息。遮挡会让真实存在的物体无法出现在当前观测中，因此系统需要结合历史、主动感知或状态估计进行判断。
3. 本体会决定传感器的位置与能力、可执行动作、可达范围、交互代价，以及智能体能够收集到的数据分布。
4. 开环执行适合环境稳定、模型准确且扰动较少的任务；闭环执行适合动态、不确定或需要持续校正的任务。实际机器人通常组合使用两者。
5. 文本计划没有自动满足坐标系、动作接口、控制频率、动力学、碰撞和安全约束，因此还需要状态估计、运动规划与控制等环节。
6. VLA 主要学习视觉、语言和机器人状态到动作的映射；它通常不覆盖机器人本体、低层控制、系统调度、安全机制和完整的失败恢复。
7. 仿真智能体需要具有明确本体、传感器、动作空间和环境动力学，并通过行动改变环境、获得新反馈和完成任务。
8. 物体识别、位姿估计、可达性判断、路径规划、碰撞检测、抓取接触、执行延迟、掉落检测和失败恢复都可能出错。
9. 反馈包括动作后的新观测、测量与结果信息；奖励是强化学习中表达目标偏好的标量信号。闭环系统只要能够根据反馈修正行为即可，不一定通过奖励学习策略。
...
- Margaret Wilson, [*Six Views of Embodied Cognition*](https://doi.org/10.3758/BF03196322), 2002.
- Leslie Pack Kaelbling, Michael L. Littman and Anthony R. Cassandra, [*Planning and Acting in Partially Observable Stochastic Domains*](https://doi.org/10.1016/S0004-3702%2898%2900023-X), 1998.
...
- Richard S. Sutton and Andrew G. Barto, [*Reinforcement Learning: An Introduction*](https://mitpress.mit.edu/9780262039246/reinforcement-learning/), 2nd Edition.
